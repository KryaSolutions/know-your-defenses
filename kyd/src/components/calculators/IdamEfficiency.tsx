...
const IdamEfficiency = () => {
    ...
    const steps = [
        {
            ...
            fields: [
                ...
                {
                    key: "sessionMonitoring",
                    label: "Session Monitoring (%)",
                    type: "percentage",
                },
                {
                    key: "passwordVaulting",
                    label: "Password Vaulting (%)",
                    type: "percentage",
                },
            ],
        },
        {
            ...
            fields: [
                ...
                {
                    key: "accessReviewCompliance",
                    label: "Access Review Compliance (%)",
                    type: "percentage",
                },
                {
                    key: "provisioningTime",
                    label: "Provisioning Time (hrs)",
                    type: "number",
                },
                {
                    key: "deprovisioningTime",
                    label: "De-Provisioning Time (hrs)",
                    type: "number",
                },
                {
                    key: "roleBasedAccess",
        ...
        },
        {
            ...
            fields: [
                ...
                {
                    key: "ssoImplementation",
                    label: "SSO Implementation (%)",
                    type: "percentage",
                },
                {
                    key: "adaptiveAuth",
                    label: "Adaptive Auth Coverage (%)",
                    type: "percentage",
                },
            ],
        },
        ...
        },
        {
            ...
            fields: [
                {
                    key: "mtDetect",
                    label: "Mean Time to Detect (hrs)",
                    type: "number",
                },
                {
                    key: "mtRespond",
                    label: "Mean Time to Respond (hrs)",
                    type: "number",
                },
                {
    ...
    const calculateResults = () => {
        ...
        const pimPamScore =
            (m.pamCoverage + m.sessionMonitoring + m.passwordVaulting) / 3;
        const igaScore =
            (m.accessReviewCompliance +
                m.roleBasedAccess +
                (100 - Math.min(100, (m.provisioningTime / 72) * 100)) +
                (100 - Math.min(100, (m.deprovisioningTime / 48) * 100))) /
            4;
        const authScore =
            (m.mfaAdoption + m.ssoImplementation + m.adaptiveAuth) / 3;
        const govScore =
            (m.policyCompliance + m.auditReadiness + m.segregationDuties) / 3;
        const irScore =
            (100 -
                Math.min(100, (m.mtDetect / 24) * 100) +
                (100 - Math.min(100, (m.mtRespond / 48) * 100)) +
                m.automatedRemediation) /
            3;

        const overallEfficiency =
            (pimPamScore + igaScore + authScore + govScore + irScore) / 5;

        const provisioningEff =
            100 - Math.min(100, (m.provisioningTime / 72) * 100);
        const deprovisioningEff =
            100 - Math.min(100, (m.deprovisioningTime / 48) * 100);
        const riskReduction =
            ((m.pamCoverage + m.mfaAdoption + m.segregationDuties) / 3) * 0.7;

        const avgIncidentCost = 75000;
        const potentialLossPrevented =
            m.securityIncidents * avgIncidentCost * (riskReduction / 100);
        const automationSavings =
            m.annualBudget * 0.05 * (m.automatedRemediation / 100);
        ...
        setTimeout(() => {
            ...
            updateCalcMetrics("idamEfficiency", buffer, setCalcMetrics);
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
                            ...
                            <div className="border rounded-lg p-4 bg-gray-50">
                                ...
                                <div className="text-xs text-gray-600">
                                    ${(results.totalValue / 1000).toFixed(0)}K
                                    value
                                </div>
                            </div>
                            ...
                            <div className="border rounded-lg p-4 bg-gray-50">
                                ...
                                <div className="text-2xl font-bold text-gray-800 mb-2">
                                    {Math.round(results.timeToValue)}
                                </div>
                                <div className="text-xs text-gray-600">
                                    days
                                </div>
                        ...
                        <div className="mb-6">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                                {[
                                    {
                                        label: "PIM/PAM",
                                        score: results.pimPamScore,
                                    },
                                    { label: "IGA", score: results.igaScore },
                                    {
                                        label: "Authentication",
                                        score: results.authScore,
                                    },
                                    {
                                        label: "Governance",
                                        score: results.govScore,
                                    },
                                    {
                                        label: "Incident Response",
                                        score: results.irScore,
                                    },
                                ].map((component) => (
                                    <div
                                        key={component.label}
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                                <div className="border rounded-lg p-3 bg-gray-50">
                                    ...
                                    <div className="text-lg font-bold text-gray-800">
                                        {results.provisioningEff.toFixed(1)}%
                                    </div>
                                ...
                                <div className="border rounded-lg p-3 bg-gray-50">
                                    ...
                                    <div className="text-lg font-bold text-gray-800">
                                        {results.deprovisioningEff.toFixed(1)}%
                                    </div>
                                    <div className="text-xs text-gray-500">
                                        vs. 48-hour baseline
                                    </div>
                                ...
                                <div className="border rounded-lg p-3 bg-gray-50">
                                    ...
                                    <div className="text-lg font-bold text-gray-800">
                                        {results.riskReduction.toFixed(1)}%
                                    </div>
                                    <div className="text-xs text-gray-500">
                                        estimated impact
                                    </div>
                                ...
                                <div className="border rounded-lg p-3 bg-gray-50">
                                    ...
                                    <div className="text-lg font-bold text-gray-800">
                                        $
                                        {(
                                            results.potentialLossPrevented /
                                            1000
                                        ).toFixed(0)}
                                        K
                                    </div>
                        ...
                        {results.igaScore < 70 && (
                            <div className="bg-orange-50 border border-orange-200 rounded-lg p-3 mb-4">
                                <div className="font-medium text-orange-800 text-sm mb-1">
                                    IGA Optimization Needed
                                </div>
                                <div className="text-xs text-orange-700">
                                    Focus on reducing provisioning times and
                                    improving access review compliance.
                                </div>
                            </div>
                        )}
